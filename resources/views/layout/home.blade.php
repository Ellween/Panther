        ...
        <div class="container-fluid d-flex flex-column justify-content-around text-light h-100 align-items-center">
          <div class="row w-100 amas" style ='width: 86% !important;'>
            @foreach($categories as $category)
              <option class ='category_id' value="{{$category->id}}">{{$category->category}}</option>
            @endforeach

              @foreach($post as $post)
                <div data-id ={{$post->id}} class="col-md-6  col-lg-4 col-sm-12  mt-3  bg-img" style ='overflow: hidden;' >
